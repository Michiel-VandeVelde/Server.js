...
export default defineConfig({
  resolve: {
    ...
    // default extension order prefers .js, so an import from a .ts test file
    // and the SAME module's own internal relative imports (resolved through
    // the compiled .js's require()) can silently load two different module
    // instances of the same file — breaking instanceof/identity checks.
    // Preferring .ts first keeps every resolution consistently on source.
