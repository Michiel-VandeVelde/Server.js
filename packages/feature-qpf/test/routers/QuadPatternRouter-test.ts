...
type Case = [string, string, string, Partial<Query>, Partial<Query>];
    ...
    describe('extractUrlParams', () => {
      describe('with an existing query', () => {
        (
          ...
          extractQueryParams(router, description, url, intent, query as Record<string, unknown>, expectedQuery as Record<string, unknown>);
        });
      });
    });
  });
