...
interface SupertestChainable {
  end(callback: (error: Error | null, res: SupertestResponse) => void): void;
}
interface SupertestAgent {
  get(path: string): SupertestChainable;
  post(path: string): SupertestChainable;
  options(path: string): SupertestChainable;
}
...
describe('TimegateController', () => {
  ...
  describe('An instance handling requests', () => {
    let controller: TimegateController & FakeController, client: SupertestAgent;
    beforeAll(() => {
      let datasource: FakeDatasource = { id: 'ds1', path: '/ds1/' };
    ...
    it('should hand over to the next controller for an unconfigured timegate', () => new Promise<void>((done) => {
    ...
    it('should end an OPTIONS request without handing over to the next controller', () => new Promise<void>((done) => {
    ...
    it('should redirect to the closest memento with Link and Vary headers', () => new Promise<void>((done) => {
    ...
    describe('for a configured timegate with no mementos', () => {
      let emptyController: TimegateController & FakeController, emptyClient: SupertestAgent;
      beforeAll(() => {
    ...
    describe('for a timegate whose timemap carries a custom original base URL', () => {
      let customClient: SupertestAgent;
      beforeAll(() => {
        let datasource: FakeDatasource = { id: 'ds1', path: '/ds1/' };
