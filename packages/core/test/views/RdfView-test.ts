...
// Protected members intentionally exercised directly by these tests.
function internal(view: RdfView): any {
  return view;
}
  ...
  describe('An RdfView instance', () => {
    describe('without a _generateRdf implementation', () => {
      it('should throw an error when calling _generateRdf', () => {
        expect(() => { internal(new RdfView('', { dataFactory }))._generateRdf({}, noop, noop, noop); })
    ...
    describe('_renderViewExtension', () => {
      it('should call _generateRdf on an extension that implements it', () => {
        let extension = { _generateRdf: sinon.spy() },
            view = internal(new RdfView('', { dataFactory })),
            options = { writer: { data: noop, meta: noop } }, done = noop;
        view._renderViewExtension(extension, options, {}, {}, done);
        expect(extension._generateRdf.calledOnce).toBe(true);
        expect(extension._generateRdf.calledWith(options, options.writer.data, options.writer.meta, done)).toBe(true);
      });

      it('should do nothing for an extension that does not implement _generateRdf', () => {
        let extension = {},
            view = internal(new RdfView('', { dataFactory })),
            options = { writer: { data: noop, meta: noop } };
    ...
    describe('_addDatasources', () => {
      it('should skip datasources without a url', () => {
        let view = internal(new RdfView('', { dataFactory })), metadata = sinon.spy();
    ...
    describe('_createN3Writer', () => {
      it('should preserve a metadata quad\'s own non-default graph', () => new Promise<void>((done) => {
        let view = internal(new RdfView('', { dataFactory })), written = '';
        let response = { write: (data: string) => { written += data; } };
      ...
      it('should write nothing when the underlying N3 writer errors', () => new Promise<void>((done) => {
        ...
        N3.Writer.prototype.end = function (callback: any) { callback(new Error('failed')); };
        try {
          let view = internal(new RdfView('', { dataFactory })), written: string;
          let response = { write: (data: string) => { written = data; } };
    ...
    describe('_createJsonLdWriter', () => {
      function collect() {
        ...
        return { response, getOutput: () => output };
      }

      it('should not set @base when the prefixes have no base entry', () => new Promise<void>((done) => {
        let view = internal(new RdfView('', { dataFactory })), { response, getOutput } = collect();
      ...
      it('should call back with an error when the underlying JSON-LD serializer errors', () => new Promise<void>((done) => {
        let view = internal(new RdfView('', { dataFactory })), { response } = collect(), callCount = 0;
        let writer = view._createJsonLdWriter({ prefixes: {} }, response, (error: Error) => {
          if (callCount++ === 0) {
