...
describe('Datasource', () => {
  ...
  describe('A Datasource instance with an initializer', () => {
    let datasource: any, initializedListener: any, errorListener: any, initResolver: any, initSpy: any;
    before(() => {
  ...
  describe('A Datasource instance with an initializer that errors synchronously', () => {
    let datasource: any, initializedListener: any, errorListener: any, error: any;
    before(() => {
      datasource = new Datasource({ dataFactory });
  ...
  describe('A derived Datasource instance', () => {
    let datasource: any = new Datasource({ dataFactory });
    Object.defineProperty(datasource, 'supportedFeatures', {
      enumerable: true,
  ...
  describe('A Datasource instance with a graph property', () => {
    let datasource: any = new Datasource({
      dataFactory,
    ...
    datasource.initialize();
    datasource._executeQuery = sinon.spy((query: any, destination: any) => {
    ...
    it('should move triples in the default graph to the given graph', (done) => {
      let result = datasource.select({ features: { custom: true } }, done), quads: any[] = [];
      result.on('data', (q: any) => { quads.push(q); });
      result.on('end', () => {
        let matchingquads = [
