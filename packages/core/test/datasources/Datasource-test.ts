...
// Tests exercise Datasource's protected internals (_fetch/_initialize/_executeQuery) directly;
// intersecting with an index signature doesn't bypass TS's protected-access check on the
// original declaration, so this is a plain `any` alias instead.
type TestableDatasource = any;
...
describe('Datasource', () => {
  ...
  describe('A Datasource instance', () => {
    let datasource = new Datasource({ dataFactory }) as TestableDatasource;
    ...
    it('should throw an error when trying to execute an unsupported query', () => new Promise<void>((done) => {
      datasource.select({ features: { a: true, b: true } }, (error?: Error) => {
    ...
    describe('fetching a resource', () => {
      it('fetches an existing resource', () => new Promise<void>((done) => {
        let result = datasource._fetch({ url: 'file://' + exampleFile }), buffer = '';
        result.on('data', (d: string) => { buffer += d; });
        result.on('end', () => {
      ...
      it('assumes file:// as the default protocol', () => new Promise<void>((done) => {
      ...
      it('emits an error when the protocol is unknown', () => new Promise<void>((done) => {
      ...
      it('emits an error on the datasource when no error listener is attached to the result', () => new Promise<void>((done) => {
      ...
      it('does not emit an error on the datasource when an error listener is attached to the result', () => new Promise<void>((done) => {
        let result = datasource._fetch({ url: exampleFile + 'notfound' });
        result.on('error', (error: Error) => {
          expect(error.message).toContain('ENOENT: no such file or directory');
          done();
        ...
        datasource.on('error', (error: Error) => {
          done(error as unknown as undefined);
        });
      }));

      it('fetches an http(s) resource via the configured request function', () => new Promise<void>((done) => {
        ...
        let httpDatasource = new Datasource({ dataFactory, request: fakeRequest as unknown as typeof import('request') }) as TestableDatasource;
      ...
      it('emits an error for an http(s) response with a non-success status code', () => new Promise<void>((done) => {
    ...
    describe('when closed with a callback', () => {
      it('should invoke the callback', () => new Promise<void>((done) => {
  ...
  describe('A disabled Datasource instance', () => {
    ...
    it('should initialize immediately without becoming queryable', () => new Promise<void>((done) => {
  ...
  describe('A Datasource instance with an initializer', () => {
    let datasource: TestableDatasource, initializedListener: ReturnType<typeof sinon.stub>, errorListener: ReturnType<typeof sinon.stub>,
        initResolver: () => void, initSpy: ReturnType<typeof sinon.spy>;
    beforeAll(() => {
      datasource = new Datasource({ dataFactory }) as TestableDatasource;
      datasource._initialize = () => new Promise<void>((resolve) => { initResolver = resolve; });
    ...
    describe('after construction', () => {
      ...
      it('should error when trying to query', () => new Promise<void>((done) => {
        datasource.select({}, (error?: Error) => {
    ...
    describe('after the initializer calls the callback', () => {
      ...
      it('should allow querying', () => new Promise<void>((done) => {
        datasource.select({}, (error?: Error) => {
  ...
  describe('A Datasource instance with an initializer that errors synchronously', () => {
    let datasource: TestableDatasource, initializedListener: ReturnType<typeof sinon.stub>, errorListener: ReturnType<typeof sinon.stub>, error: Error;
    beforeAll(() => {
      datasource = new Datasource({ dataFactory }) as TestableDatasource;
    ...
    describe('after the initializer calls the callback', () => {
      it('should have called the initializer', () => {
        expect((datasource._initialize as unknown as ReturnType<typeof sinon.spy>).calledOnce).toBe(true);
      });
  ...
  describe('A derived Datasource instance', () => {
    ...
    datasource._executeQuery = sinon.stub() as unknown as typeof datasource._executeQuery;
    datasource.initialize();
    ...
    it('should attach an error listener on select if one was passed', () => {
      ...
      let result = datasource.select({ features: {} }, onError as unknown as (error: Error) => void);
  ...
  describe('A Datasource instance with a graph property', () => {
    ...
    }) as TestableDatasource;
    Object.defineProperty(datasource, 'supportedFeatures', {
      enumerable: true,
      value: { custom: true },
    });
    datasource.initialize();
    datasource._executeQuery = sinon.spy((query: unknown, destination: { _push: (q: unknown) => void; close: () => void }) => {
      ...
      destination.close();
    }) as unknown as typeof datasource._executeQuery;

    beforeEach(() => {
      (datasource._executeQuery as unknown as ReturnType<typeof sinon.spy>).reset();
    });

    it('should move triples in the default graph to the given graph', () => new Promise<void>((done) => {
      let result = datasource.select({ features: { custom: true } }, done as unknown as (error: Error) => void), quads: unknown[] = [];
      result.on('data', (q: unknown) => { quads.push(q); });
      result.on('end', () => {
        let matchingquads = [
    ...
    it('should query the given graph as the default graph', () => {
      ...
      let spy = datasource._executeQuery as unknown as ReturnType<typeof sinon.spy>;
      expect(spy.args[0][0].features).toEqual({ custom: true });
      expect(spy.args[0][0].graph.equals(dataFactory.defaultGraph())).toBe(true);
    });
    ...
    it('should query the default graph as the empty graph', () => {
      ...
      let spy = datasource._executeQuery as unknown as ReturnType<typeof sinon.spy>;
      expect(spy.args[0][0].features).toEqual({ custom: true });
      expect(spy.args[0][0].graph.equals(dataFactory.namedNode('urn:ldf:emptyGraph'))).toBe(true);
    });
  });
});
